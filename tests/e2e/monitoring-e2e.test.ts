...
const skipInCI = process.env.CI && !process.env.ENABLE_E2E_TESTS;

test.describe.skipIf(skipInCI)("@P0 Monitoring End-to-End Tests", () => {
  ...
  const TEST_CONFIG = {
    ...
    edgeFunctionUrl:
      process.env.TEST_EDGE_FUNCTION_URL ||
      "http://localhost:54321/functions/v1/parse-request",
    monitoringDashboard:
      process.env.TEST_GRAFANA_URL || "http://localhost:3000",
  ...
  test.beforeEach(async ({ browser }) => {
    ...
    page.on("response", (response) => {
      if (
        ...
        response.url().includes("/traces")
      ) {
        console.log(
          `Monitoring response: ${response.url()} - ${response.status()}`,
        );
      }
    });
  });
  ...
  test.describe("1.7-E2E-001: End-to-End Request Trace Collection", () => {
    test("should trace complete parse request flow from Edge Function to NLP Service", async () => {
      // Arrange
      const testInput =
        'Create task "Test E2E monitoring" for @testuser due tomorrow #monitoring';
      ...
      if (TEST_CONFIG.jaegerUI !== "http://localhost:16686") {
        ...
        const traceResults = await page.locator(
          '[data-testid="trace-results"]',
        );
    ...
    test("should collect performance metrics for request processing", async () => {
      ...
      const performanceTestCases = [
        ...
        {
          input:
            "Complex analysis with multiple @mentions and #tags requiring OpenAI processing",
      ...
      for (const testCase of performanceTestCases) {
        ...
        console.log(
          `${testCase.expectedType} parse response time: ${responseTime}ms`,
        );
      }
    });
    ...
    test("should capture runtime errors and exceptions", async () => {
      ...
      for (const testCase of errorTestCases) {
        ...
        if (status >= 400) {
          ...
          expect(responseData).toHaveProperty("error");
          console.log(
            `Runtime error captured for ${testCase.description}: ${responseData.error}`,
          );
        }
      }

...
function validateEnvironment(envFile) {
  ...
  if (!existsSync(envFile)) {
    ...
    console.error('Create the environment file or specify a different one with --env-file');
    return false;
  }
  ...
  try {
    ...
    if (result.error) {
      console.warn('⚠️  1Password CLI not found. Secrets will not be injected.');
      console.warn('Install 1Password CLI and authenticate to use secure secret management.');
    }
  } catch (error) {
...
function runTestSuite(suiteName, config, options) {
  return new Promise((resolve, reject) => {
    ...
    const timeoutId = setTimeout(() => {
      console.error(`❌ Test suite ${suiteName} timed out after ${config.timeout}ms`);

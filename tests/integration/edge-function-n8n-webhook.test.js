...
// Test configuration
const N8N_WEBHOOK_URL = process.env.N8N_WEBHOOK_URL || 'https://n8n-rrrs.sliplane.app/webhook/telegram-task-creation';
...
before(() => {
  validateTestConfig();
  assert.ok(N8N_WEBHOOK_URL, 'N8N_WEBHOOK_URL must be configured for integration tests');
});
...
describe('Edge Function → n8n Webhook Integration', () => {
  ...
  describe('Architecture Pattern Verification', () => {
    test('should implement "Reflex + Brain" pattern correctly', async () => {
      ...
      assert.ok(
        edgeFunctionResponse.status === 200 || edgeFunctionResponse.status === 202,
    ...
    test('should handle network timeout scenarios', async () => {
      ...
      try {
        await fetch(N8N_WEBHOOK_URL, {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            update: { message: { text: 'timeout test', chat: { id: 1 }, from: { id: 1 }, message_id: 1 } }
          }),
...
describe('Integration Test Configuration', () => {
  test('should have all required environment variables for integration testing', () => {
    ...
    assert.ok(N8N_WEBHOOK_URL.includes('webhook'), 'N8N_WEBHOOK_URL should be a webhook endpoint');
    assert.ok(testConfig.supabase.url, 'Supabase URL should be configured');
    assert.ok(testConfig.endpoints.telegramWebhook, 'Telegram webhook endpoint should be configured');
  });

  test('should have reasonable performance thresholds', () => {
    assert.ok(PERFORMANCE_THRESHOLD_MS >= 50, 'Performance threshold should allow for network latency');
    assert.ok(PERFORMANCE_THRESHOLD_MS <= 500, 'Performance threshold should enforce responsiveness');
    assert.ok(E2E_THRESHOLD_MS >= 1000, 'E2E threshold should allow for processing time');
  });
});
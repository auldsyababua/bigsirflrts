...
before(() => {
  validateTestConfig();
  assert.ok(N8N_WEBHOOK_URL, 'N8N_WEBHOOK_URL must be configured for performance tests');
});
...
describe('Edge Function Performance Regression Tests', () => {
  ...
  test('should maintain baseline performance under normal load', async () => {
    ...
    if (stats.p95 > PERFORMANCE_BASELINE.edgeFunction.p95 * 1.3) {
      console.warn(`⚠️  Performance regression detected: 95th percentile increased from ${PERFORMANCE_BASELINE.edgeFunction.p95}ms to ${stats.p95}ms`);
    }
  });
...
describe('n8n Webhook Performance Regression Tests', () => {
  ...
  test('should maintain sub-200ms response times under load', async () => {
    ...
    if (stats.p95 > PERFORMANCE_BASELINE.n8nWebhook.p95 * 1.4) {
      console.warn(`⚠️  n8n webhook performance regression: 95th percentile increased from ${PERFORMANCE_BASELINE.n8nWebhook.p95}ms to ${stats.p95}ms`);
    }
  });
  ...
  test('should handle occasional usage bursts (simulated team meetings)', async () => {
    ...
    const burstRequests = Array.from({ length: 6 }, () =>  // 6 users creating tasks simultaneously
      measureRequest(
        N8N_WEBHOOK_URL,
        {
          method: 'POST',

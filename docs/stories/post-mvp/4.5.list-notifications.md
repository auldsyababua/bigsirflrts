...
Implement a comprehensive notification system for list-related events including
reminders, due dates, sharing updates, completion tracking, and collaborative
changes, delivered through multiple channels with smart batching and user
preferences.
...
- **Dependencies**:
  ...
  - Story 4.4 (List Sharing & Permissions) - Required for collaboration
    notifications
...
class NotificationService extends EventEmitter {
  ...
  async scheduleNotification(
    ...
    const shouldBatch = this.shouldBatchNotification(notification, preferences);
  ...
  private formatTelegramMessage(notification: ListNotification): string {
    ...
    if (data.dueDate) {
      ...
      if (duration.total('hours') < 24) {
        message += `\n⏰ Due: ${this.formatRelativeTime(duration)}`;
      } else {
        message += `\n📅 Due: ${this.formatDate(due)}`;
      }
    }
  ...
  private buildTelegramKeyboard(notification: ListNotification): any {
    ...
    keyboard.inline_keyboard.push([
      {
        text: '✅ Mark as Read',
        callback_data: `notif_read:${notification.id}`,
      },
      {
        text: '🔕 Mute Similar',
        callback_data: `notif_mute:${notification.type}`,
      },
    ]);
  ...
  async generateDigest(
    ...
    const { data: summary } = await this.supabase.rpc(
  ...
  async trackEffectiveness(
    ...
    await this.supabase.from('notification_analytics').upsert(updates);
...
class TelegramNotificationHandler {
  ...
  async handlePreferencesCommand(ctx: Context): Promise<void> {
    const keyboard = new InlineKeyboard()
      .row(
        InlineKeyboard.text('🔔 Channels', 'pref_channels'),
        InlineKeyboard.text('🌙 Quiet Hours', 'pref_quiet')
      )
      .row(
        InlineKeyboard.text('📦 Batching', 'pref_batch'),
        InlineKeyboard.text('🎯 Filters', 'pref_filters')
      )
      .row(
        InlineKeyboard.text('📊 Digest Settings', 'pref_digest'),
        InlineKeyboard.text('🌍 Language', 'pref_language')
      );
  ...
  async configureQuietHours(ctx: Context): Promise<void> {
    ...
    const options = [
      { label: '🌃 Night (10 PM - 7 AM)', start: '22:00', end: '07:00' },
      { label: '🌙 Late Night (11 PM - 6 AM)', start: '23:00', end: '06:00' },
      {
        label: '💼 Work Hours Only (9 AM - 5 PM)',
        start: '17:00',
        end: '09:00',
      },
      { label: '🎯 Custom Hours', custom: true },
    ];

    options.forEach((opt) => {
      if (opt.custom) {
        keyboard.row(InlineKeyboard.text(opt.label, 'quiet_custom'));
      } else {
        keyboard.row(
          InlineKeyboard.text(opt.label, `quiet_set:${opt.start}:${opt.end}`)
        );
      }
    });

    keyboard.row(
      InlineKeyboard.text('❌ Disable Quiet Hours', 'quiet_disable'),
      InlineKeyboard.text('← Back', 'pref_back')
    );
  ...
  async showNotificationHistory(ctx: Context): Promise<void> {
    ...
    keyboard.row(
      InlineKeyboard.text('🗑 Clear All', 'notif_clear_all'),
      InlineKeyboard.text('⚙️ Settings', 'notif_settings')
    );
  ...
  async createCustomSubscription(ctx: Context): Promise<void> {
    await ctx.reply(
      '🔔 *Create Custom Alert*\n\n' +
        ...
        'Examples:\n' +
        '• "When any task is overdue"\n' +
        '• "When someone completes an item I assigned"\n' +
        '• "Daily summary at 9 AM"\n' +
        '• "When a high-priority task is added"',
      { parse_mode: 'Markdown' }
    );
...
class NotificationTriggerEngine {
      ...
      action: async (item) => {
        ...
        await this.notificationService.scheduleNotification({
          ...
          payload: {
            ...
            actions: [
              { id: 'complete', label: '✅ Complete Now' },
    ...
    this.addRule({
      ...
      action: async (event) => {
        await this.notificationService.scheduleNotification({
          ...
          priority: 'normal',
          payload: {
            title: `🎯 ${event.completionPercentage}% Complete!`,
  ...
  async processEvent(event: SystemEvent): Promise<void> {
    const applicableRules = this.rules.filter(
      (rule) => rule.trigger === 'event' && rule.event === event.type
    );
  ...
  describe('Delivery', () => {
    ...
    it('should batch low-priority notifications', async () => {
      const notifications = Array(5)
        .fill(null)
        .map(() => createTestNotification({ priority: 'low' }));
...
1. ✅ Notifications delivered based on user preferences
2. ✅ Quiet hours respected for non-urgent notifications
3. ✅ Smart batching reduces notification fatigue
4. ✅ Multi-channel delivery with fallback
5. ✅ Analytics track notification effectiveness
6. ✅ Digest summaries provide valuable insights

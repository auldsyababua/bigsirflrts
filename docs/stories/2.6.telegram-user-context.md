...
Implement session management and user context tracking for the Telegram bot,
maintaining user state, preferences, and conversation flow across interactions.
...
class WorkflowManager {
  ...
  async continue(
    ...
    // Validate input
    if (step.validator) {
      const validationResult = step.validator(
        input,
      ...
      if (!validationResult) {
        await ctx.reply(
          step.errorMessage || 'Invalid input. Please try again.'
        );
        return;
      }
    }
  ...
  async cancel(ctx: Context, session: UserSession): Promise<void> {
    ...
    await ctx.reply('❌ Workflow cancelled.');
  }
  ...
  private async executeStep(
    ...
    if (
      step.skipCondition &&
      step.skipCondition(session.context.workflowData)
    ) {
    ...
    await ctx.reply(prompt, {
      reply_markup: {
        inline_keyboard: [
          [{ text: '❌ Cancel', callback_data: 'workflow:cancel' }],
        ],
      },
    });
  }
  ...
  private async handleTimeout(
    ...
    await ctx.reply('⏰ Workflow timed out. Please start again.');
  }
...
const createTaskWorkflow: WorkflowDefinition = {
  ...
  steps: [
    ...
    {
      id: 'description',
      prompt: '📋 Add a description (or type "skip"):',
      transformer: (input) => (input === 'skip' ? null : input),
    },
    {
      id: 'dueDate',
      prompt: '📅 When is it due? (e.g., "tomorrow at 3pm" or "skip"):',
      ...
      errorMessage:
        'Could not understand the date. Try "tomorrow", "next Monday", etc.',
    },
    {
      id: 'priority',
...
bot.command('settings', async (ctx) => {
  const keyboard = {
    inline_keyboard: [
      [{ text: '🌍 Timezone', callback_data: 'pref:timezone' }],
      [{ text: '📅 Date Format', callback_data: 'pref:dateformat' }],
      [{ text: '🔔 Notifications', callback_data: 'pref:notifications' }],
      [{ text: '⏰ Working Hours', callback_data: 'pref:workinghours' }],
      [{ text: '📁 Default Project', callback_data: 'pref:project' }],
    ],
  };
...
async function contextAwareMiddleware(ctx: Context, next: NextFunction) {
  ...
  if (session?.conversation.pendingAction) {
    ...
    if (new Date() > action.expiresAt) {
      ...
      await ctx.reply('⏰ Previous action expired. Please start again.');
      return next();
    }
...
1. ✅ Sessions persist across bot restarts
2. ✅ User preferences are applied to all commands
3. ✅ Multi-step workflows maintain state correctly
4. ✅ Session analytics track user activity
5. ✅ Inactive sessions are cleaned up automatically

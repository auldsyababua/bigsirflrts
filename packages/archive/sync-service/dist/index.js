...
const supabase = (0, supabase_js_1.createClient)(process.env.SUPABASE_URL || 'https://thnwlykidzhrsagyjncc.supabase.co', process.env.SUPABASE_SERVICE_KEY || process.env.SUPABASE_ANON_KEY);
...
const OPENPROJECT_PROJECT_ID = Number.parseInt(String(process.env.OPENPROJECT_PROJECT_ID || ''), 10);
...
const openprojectAPI = axios_1.default.create({
...
openprojectAPI.interceptors.request.use((config) => {
    // Add X-Request-ID if not already present
    if (!config.headers['X-Request-ID']) {
        config.headers['X-Request-ID'] = (0, crypto_1.randomUUID)();
    }
    // Log request (redact auth header)
    if (process.env.NODE_ENV === 'development') {
        console.log(`→ ${config.method?.toUpperCase()} ${config.url} [${config.headers['X-Request-ID']}]`);
    }
    return config;
}, (error) => {
...
openprojectAPI.interceptors.response.use((response) => {
    // Log successful responses in development
    if (process.env.NODE_ENV === 'development') {
        console.log(`← ${response.status} ${response.config.method?.toUpperCase()} ${response.config.url} [${response.config.headers['X-Request-ID']}]`);
    }
    return response;
}, (error) => {
...
const MAX_IDEMPOTENCY_CACHE_SIZE = parseInt(process.env.OPENPROJECT_IDEMPOTENCY_CACHE_SIZE || '1000', 10);
const idempotencyCache = new LRUCache(MAX_IDEMPOTENCY_CACHE_SIZE);
...
async function retryWithBackoff(fn, options = {}) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            console.warn(`${operationName} failed (attempt ${attempt + 1}), retrying in ${backoffMs}ms... [${correlationId}]`);
...
async function initializeDictionaries() {
    ...
    try {
        ...
        await retryWithBackoff(async (correlationId) => {
        ...
        if (statusNewId > 0)
            statusCache.set('new', statusNewId);
        const statusInProgressId = safeParseInt(process.env.OPENPROJECT_STATUS_IN_PROGRESS_ID, 0);
        if (statusInProgressId > 0)
            statusCache.set('in progress', statusInProgressId);
        const statusClosedId = safeParseInt(process.env.OPENPROJECT_STATUS_CLOSED_ID, 0);
        if (statusClosedId > 0)
            statusCache.set('closed', statusClosedId);
        const statusRejectedId = safeParseInt(process.env.OPENPROJECT_STATUS_REJECTED_ID, 0);
        if (statusRejectedId > 0)
            statusCache.set('rejected', statusRejectedId);
        const priorityHighId = safeParseInt(process.env.OPENPROJECT_PRIORITY_HIGH_ID, 0);
        if (priorityHighId > 0)
            priorityCache.set('high', priorityHighId);
        const priorityNormalId = safeParseInt(process.env.OPENPROJECT_PRIORITY_NORMAL_ID, 0);
        if (priorityNormalId > 0)
            priorityCache.set('normal', priorityNormalId);
        const priorityLowId = safeParseInt(process.env.OPENPROJECT_PRIORITY_LOW_ID, 0);
        if (priorityLowId > 0)
            priorityCache.set('low', priorityLowId);
        const typeTaskId = safeParseInt(process.env.OPENPROJECT_TYPE_TASK_ID, 0);
        if (typeTaskId > 0)
            typeCache.set('task', typeTaskId);
        // Fail fast if critical mappings are missing
        const missingCritical = [];
        if (!statusCache.has('new'))
            missingCritical.push('status:new');
        if (!statusCache.has('in progress'))
            missingCritical.push('status:in progress');
        if (!priorityCache.has('normal'))
            missingCritical.push('priority:normal');
        if (!typeCache.has('task'))
            missingCritical.push('type:task');
        if (missingCritical.length > 0) {
            throw new Error(`Failed to load critical OpenProject mappings: ${missingCritical.join(', ')}. ` +
...
function mapPriority(supabasePriority) {
...
function mapStatus(supabaseStatus) {
...
async function syncTaskToOpenProject(task) {
    try {
        ...
        if (task.openproject_id) {
            ...
            response = await retryWithBackoff(async (correlationId) => {
                return await openprojectAPI.patch(`/work_packages/${task.openproject_id}`, workPackageData, {
                    headers: {
            ...
            response = await retryWithBackoff(async (correlationId) => {
                return await openprojectAPI.post(`/projects/${OPENPROJECT_PROJECT_ID}/work_packages`, workPackageData, {
                    headers: {
                        'X-Request-ID': correlationId,
        ...
        await supabase
            ...
            .eq('id', task.id);
        console.log(`✅ Successfully synced task ${task.id} to OpenProject work package ${response.data.id}`);
